...
in a terminal.  In Windows, you will want to press the windows key (or click the start button), type 'cmd', then right-click and run as administrator.  Then type `C:\Program Files\R\R-3.3.2\bin\R.exe` and press enter -- you may need to adjust the filepath for your version of R if it's not 3.3.2 (hint hint -- [tab completion](https://www.howtogeek.com/195207/use-tab-completion-to-type-commands-faster-on-any-operating-system/)), then follow the steps [here](https://irkernel.github.io/installation/#binary-panel):
